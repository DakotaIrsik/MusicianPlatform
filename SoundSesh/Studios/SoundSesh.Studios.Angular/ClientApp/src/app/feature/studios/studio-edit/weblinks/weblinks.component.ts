...
export class WeblinksComponent extends Masks implements OnInit {
  ...
  ngOnInit(): void {
    this.formattedWebsiteLink = (this.studio.websitelink) ? this.studio.websitelink.replace('http://', '') : '';
    this.formattedAudioClipLink = (this.studio.audiocliplink) ? this.studio.audiocliplink.replace('http://', '') : '';
  }

  onSubmit() {
    this.studio.websitelink = 'http://' + this.formattedWebsiteLink;
    this.studio.audiocliplink = 'http://' + this.formattedAudioClipLink;
